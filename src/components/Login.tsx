...
import { FiUser, FiEye, FiEyeOff, FiLock, FiArrowLeft, FiX } from "react-icons/fi";
...
export default function LoginP() {
  ...
  // 🔹 useEffect para focar o input de senha corretamente no iOS/Safari
  useEffect(() => {
    if (step === "senha") {
      setTimeout(() => {
        senhaRef.current?.focus({ preventScroll: true });
      }, 100); // delay mínimo para garantir que o input foi renderizado
    }
  }, [step]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (step === "cpf") {
      ...
      setStep("senha"); // 🔹 agora o foco será tratado pelo useEffect
      return;
    }
  ...
  const handleCpfKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      if (validarCpf(cpf)) {
        setStep("senha"); // 🔹 foco também via useEffect
      } else {
        toast.error("CPF inválido", {
          position: "top-center",
          style: { background: "#f56565", color: "#fff", fontWeight: "bold" },
            ...
            <>
              ...
              <label className="block relative">
                ...
                <button
                  ...
                  disabled={loading} // ✅ trava toggle senha
                >
